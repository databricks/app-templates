...
async def get_memory(ctx: RunContextWrapper[MemoryContext], path: str) -> str:
    ...
    try:
        entry = _ws().api_client.do("GET", _entries(":get"), query={"scope": _scope(ctx), "path": path})
    except DatabricksError as e:
...
async def list_memories(ctx: RunContextWrapper[MemoryContext]) -> str:
    ...
    resp = _ws().api_client.do("GET", _entries(), query={"scope": _scope(ctx)})
...
async def update_memory(
    ...
    try:
        _ws().api_client.do("PATCH", _entries(), body={"scope": _scope(ctx), "path": path, **op})
    except DatabricksError as e:
        if e.error_code == "NOT_FOUND":
...
async def delete_memory(ctx: RunContextWrapper[MemoryContext], path: str) -> str:
    ...
    try:
        _ws().api_client.do("DELETE", _entries(), query={"scope": _scope(ctx), "path": path})
    except DatabricksError as e:
        if e.error_code == "NOT_FOUND":
