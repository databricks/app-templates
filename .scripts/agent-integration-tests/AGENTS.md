...
**Lakebase access grants**: For memory-backed templates, the deploy phase grants the app's service principal access to the Lakebase instance by invoking the template's own `scripts/grant_lakebase_permissions.py` as a subprocess. This exercises the exact code path users follow and avoids drift between test grants and the script shipped in the template. The test reads `LAKEBASE_AGENT_MEMORY_SCHEMA` from the template's `databricks.yml` and forwards it via env so the script grants on the same schema the deployed app reads/writes. `template.memory_type` (`"langgraph"` or `"openai"`) is passed via `--memory-type`.
