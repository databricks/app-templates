...
The resource key `agent_langgraph_advanced` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind agent_langgraph_advanced <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_langgraph_advanced
```
...
| App running old code after deploy | Run `databricks bundle run agent_langgraph_advanced` after deploy |
