...
Options:
    ...
    --lakebase-autoscaling-endpoint ENDPOINT  Autoscaling endpoint resource path
...
def create_lakebase_instance(profile_name: str) -> dict:
    ...
    try:
        ...
        branch_info = validate_lakebase_autoscaling(profile_name, project_short, branch_name)
        return {
            "type": "autoscaling",
...
def _branch_resource_path(project: str, branch: str) -> str:
    if branch.startswith("projects/"):
        return branch
    return f"projects/{project}/branches/{branch}"


def _child_resource_path(parent: str, child_type: str, name: str) -> str:
    if name.startswith("projects/"):
        return name
    return f"{parent}/{child_type}/{name}"


def _parse_autoscaling_endpoint(endpoint: str) -> tuple[str, str] | None:
    match = re.match(
...
def _extract_default_database_resource(
    profile_name: str,
    project: str,
    branch: str,
) -> str | None:
    branch_resource = _branch_resource_path(project, branch)
    result = run_command(
        [
            "databricks",
            "-p",
            ...
            "api",
            "get",
            f"/api/2.0/postgres/{branch_resource}/databases",
            "--output",
            "json",
        ],
        check=False,
    ...
    if result.returncode != 0 or not result.stdout:
        print_error(
            "Could not fetch Lakebase autoscaling databases for bundle resource configuration."
        )
        return None

    try:
        databases_data = json.loads(result.stdout)
    except json.JSONDecodeError:
        print_error("Could not parse Lakebase autoscaling databases response.")
        return None

    databases = databases_data.get("databases", [])
    if not databases:
        print_error("No Lakebase autoscaling databases found for the selected branch.")
        return None

    def _database_score(database: dict) -> int:
        text = json.dumps(database, default=str)
        return 0 if "databricks_postgres" in text else 1

    database = sorted(databases, key=_database_score)[0]
    name = database.get("name") or database.get("id") or database.get("database_id")
    if name:
        return _child_resource_path(branch_resource, "databases", name)
    print_error("Could not determine Lakebase autoscaling database resource path.")
    return None
...
def validate_lakebase_autoscaling(profile_name: str, project: str, branch: str) -> dict | None:
    ...
    # Fetch endpoint host for PGHOST and endpoint resource for local server config.
    pg_host = ""
    endpoint_resource = ""
    result = run_command(
        [
            ...
            "api",
            "get",
            f"/api/2.0/postgres/{branch_resource}/endpoints",
            "--output",
            "json",
        ],
        check=False,
    ...
    if result.returncode == 0 and result.stdout:
        try:
            ...
            if endpoints:
                ...
                if endpoint_name:
                    endpoint_resource = _child_resource_path(
                        branch_resource, "endpoints", endpoint_name
                    )
                host = (
    ...
    if not endpoint_resource:
        print_error("No Lakebase autoscaling endpoint found for the selected branch.")
        return None

    database_resource = _extract_default_database_resource(profile_name, project, branch)
...
def setup_lakebase(
    ...
    if autoscaling_endpoint:
        parsed = _parse_autoscaling_endpoint(autoscaling_endpoint)
        if not parsed:
            print_error(
                "--lakebase-autoscaling-endpoint must be a full resource path: "
                "projects/<project>/branches/<branch>/endpoints/<endpoint>"
            )
            sys.exit(1)
        autoscaling_project, autoscaling_branch = parsed
        print(f"Using autoscaling Lakebase endpoint: {autoscaling_endpoint}")
        branch_info = validate_lakebase_autoscaling(
            profile_name, autoscaling_project, autoscaling_branch
        )
        if not branch_info:
            sys.exit(1)
        ...
        if branch_info.get("host"):
            update_env_file("PGHOST", branch_info["host"])
    ...
    if autoscaling_project and autoscaling_branch:
        ...
        if not branch_info:
            sys.exit(1)
        update_env_file("LAKEBASE_AUTOSCALING_ENDPOINT", branch_info.get("endpoint", ""))
        update_env_file("LAKEBASE_AUTOSCALING_PROJECT", "")
        update_env_file("LAKEBASE_AUTOSCALING_BRANCH", "")
        update_env_file("LAKEBASE_INSTANCE_NAME", "")
...
def _replace_lakebase_resource(content: str, lakebase_config: dict) -> str:
    ...
    For provisioned: writes the database resource block.
    For autoscaling: writes the postgres resource block used by Databricks Apps.
    """
    LAKEBASE_COMMENTS = {
    ...
    result = []
    i = 0
    found_lakebase_resource = False
    resource_indent = None

    def append_lakebase_resource(indent: str) -> None:
        if lakebase_config["type"] == "provisioned":
        ...
        branch_resource = lakebase_config.get("branch_resource") or _branch_resource_path(
            lakebase_config["project"], lakebase_config["branch"]
        )
        database_resource = lakebase_config.get("database_resource") or (
            f"{branch_resource}/databases/databricks_postgres"
        )
        result.append(f"{indent}- name: 'postgres'")
        result.append(f"{indent}  postgres:")
        result.append(f"{indent}    branch: '{branch_resource}'")
        result.append(f"{indent}    database: '{database_resource}'")
        result.append(f"{indent}    permission: 'CAN_CONNECT_AND_CREATE'")
    ...
    while i < len(lines):
        ...
        # Match commented-out Lakebase resource lines
        if re.match(r"\s*#\s*- name: ['\"]?(?:database|postgres)['\"]?", stripped):
            found_lakebase_resource = True
            if resource_indent is None:
                for prev in reversed(result):
                    m = re.match(r"^(\s+)- name:", prev)
                    if m:
            ...
            append_lakebase_resource(resource_indent or "        ")
            continue

        # Match an uncommented Lakebase resource from a previous run
        if re.match(r"\s*- name: ['\"]?(?:database|postgres)['\"]?", stripped):
            found_lakebase_resource = True
            if resource_indent is None:
                m = re.match(r"^(\s+)- name:", line)
                if m:
    ...
    # If no existing Lakebase resource was found, append after the last resource entry.
    if not found_lakebase_resource:
        ...
        if insert_idx is not None:
            ...
            temp_result = result
            result = []
            append_lakebase_resource(indent)
            new_lines = result
            result = temp_result
...
Examples:
    ...
    uv run quickstart --lakebase-autoscaling-endpoint projects/proj/branches/br/endpoints/primary  # Autoscaling endpoint
    ...
    parser.add_argument(
        ...
        help=(
            "Autoscaling Lakebase endpoint resource path "
            "projects/<project>/branches/<branch>/endpoints/<endpoint>"
        ),
        metavar="ENDPOINT",
    )
