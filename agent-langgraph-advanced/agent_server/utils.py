...
FAKE_RESPONSE_ID = f"resp_placeholder_{uuid4().hex[:16]}"


def replace_fake_id(obj: Any, real_id: str) -> Any:
    """Recursively replace FAKE_RESPONSE_ID with real_id in dicts/lists/strings."""
    if isinstance(obj, dict):
        return {k: replace_fake_id(v, real_id) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [replace_fake_id(item, real_id) for item in obj]
    elif isinstance(obj, str) and obj == FAKE_RESPONSE_ID:
        return real_id
    return obj
...
async def process_agent_astream_events(
    ...
    def _start_turn():
        nonlocal in_turn, turn_output_items, output_index
        in_turn = True
        turn_output_items = []
        output_index = 0
