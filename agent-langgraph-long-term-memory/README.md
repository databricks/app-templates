...
4. Configure long-term memory by linking a Lakebase instance for conversation store and checkpointing
...
4. **Create and link a Lakebase instance for long-term memory**

   This agent uses Lakebase for long-term memory, enabling the agent to remember information about users across conversations.
   ...
   The agent uses `DatabricksStore` to persist user memories. Memories are stored per-user using the `user_id` from the chat context.
...
### Testing Long-Term Memory
   ...
   This agent supports long-term memory - it can remember information about users across conversations. To use this feature, include a `context` object with `user_id` in your requests.
   ...
   **Step 2: Retrieve the memory**
   ...
   In a new conversation (or later), ask the agent to recall the information:
   ```bash
   curl -X POST http://localhost:8000/invocations \
     -H "Content-Type: application/json" \
     -d '{
   ...
   The agent will use the same `user_id` to look up previously saved memories and respond with "Your favorite color is blue."
   ...
   **Additional examples:**
   ...
   - Save multiple memories:
     ...
     curl -X POST http://localhost:8000/invocations \
       -H "Content-Type: application/json" \
       -d '{
         "input": [{"role": "user", "content": "Remember that I live in San Francisco and work as a software engineer"}],
         "context": {"user_id": "test@example.com"}
       }'
     ```

   - Ask about saved information:
     ```bash
     curl -X POST http://localhost:8000/invocations \
       -H "Content-Type: application/json" \
       -d '{
   ...
   - Delete a memory:
     ...
     curl -X POST http://localhost:8000/invocations \
       -H "Content-Type: application/json" \
       -d '{
         "input": [{"role": "user", "content": "Please forget my favorite color"}],
         "context": {"user_id": "test@example.com"}
       }'
     ```

   > **Note:** Each `user_id` has its own isolated memory namespace. Memories saved for one user are not accessible by other users.
...
1. **Set up authentication to Databricks resources**
   ...
   - **Lakebase Instance**: Grant the App's SP permission to access your Lakebase instance for user long-term memory storage.
...
5. **Query your agent hosted on Databricks Apps**
   ...
   - Example request with long-term memory (save memory):
     ...
     curl -X POST <app-url.databricksapps.com>/invocations \
        ...
        -d '{
          "input": [{"role": "user", "content": "Remember that my favorite color is purple"}],
          "context": {"user_id": "your-email@example.com"}
        }'
     ```

   - Example request with long-term memory (retrieve a memory):
