...
6. **Query your agent hosted on Databricks Apps**
   ...
   **Using the Databricks OpenAI client (Python):**
   ...
   # With custom inputs (e.g., user ID for long-term memory)
   response = client.responses.create(
       model="apps/<app-name>",
       input=[{"role": "user", "content": "What is my favorite color?"}],
       extra_body={"custom_inputs": {"user_id": "test@example.com"}},
   )
   ```
   ...
   **Using curl:**
   ...
   ```bash
   # Request with user ID (for long-term memory)
   curl --request POST \
     --url <app-url>.databricksapps.com/responses \
     --header "Authorization: Bearer <oauth-token>" \
     --header "Content-Type: application/json" \
...
For future updates, run `databricks bundle deploy` and `databricks bundle run agent_langgraph_long_term_memory` to redeploy.
...
- **`databricks bundle deploy` fails with "An app with the same name already exists"**
  ...
  databricks bundle deployment bind agent_langgraph_long_term_memory <app-name> --auto-approve
...
- **App is running old code after `databricks bundle deploy`**
  ...
  `bundle deploy` only uploads files and configures resources. You must run `databricks bundle run agent_langgraph_long_term_memory` to actually start/restart the app with the new code.
