...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --lakebase NAME   Lakebase instance name for short-term memory"
...
echo "Setting up Lakebase instance for short-term memory..."
    ...
    if grep -q "name: LAKEBASE_INSTANCE_NAME" app.yaml 2>/dev/null; then
        ...
        if grep -q "LAKEBASE_INSTANCE_NAME=." .env.local 2>/dev/null; then
            ...
            if [ -z "$LAKEBASE_INSTANCE_NAME" ]; then
                echo "Error: Lakebase instance name is required for short-term memory"
                exit 1
            fi
        fi
