...
Use the following tables to fill any gaps:
...
Multi-Turn Scorers:
  Multi-turn scorers require:
    Session IDs: Traces must have mlflow.trace.session metadata
    List or DataFrame input: Currently only supports pre-collected traces (no predict_fn support yet)
...
Use file `agent_server/evaluation/docs/agent_strategy.md` to support your desing process
