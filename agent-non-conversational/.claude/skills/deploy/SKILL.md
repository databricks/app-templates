...
databricks bundle run agent_non_conversational
```
...
The resource key `agent_non_conversational` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind agent_non_conversational <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_non_conversational
```
...
| App running old code after deploy | Run `databricks bundle run agent_non_conversational` after deploy |
