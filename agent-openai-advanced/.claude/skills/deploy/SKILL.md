...
databricks bundle run agent_openai_agents_advanced
```
...
The resource key `agent_openai_agents_advanced` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind agent_openai_agents_advanced <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_openai_agents_advanced
```
...
| App running old code after deploy | Run `databricks bundle run agent_openai_agents_advanced` after deploy |
