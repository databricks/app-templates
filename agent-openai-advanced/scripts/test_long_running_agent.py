...
def test_background_poll(agent_url):
    ...
    assert resp.output_text
    print(f"\n[background_poll] output: {resp.output_text}")
...
def test_background_stream(agent_url):
    ...
    text_parts = []
    print("\n[background_stream] SSE events:")
    for event in stream:
        evt_type = _get_event_attr(event, "type")
        if isinstance(event, dict):
            print(f"  {event}")
        else:
            print(f"  {event.model_dump() if hasattr(event, 'model_dump') else event}")
        if evt_type == "response.output_text.delta":
            delta = _get_event_attr(event, "delta")
            if delta:
                text_parts.append(delta)
    full_text = "".join(text_parts)
    print(f"\n[background_stream] full output: {full_text}")
    ...
    for event in resumed_stream:
        ...
        if _get_event_attr(event, "type") == "response.output_text.delta":
            ...
            if delta:
                text_parts.append(delta)
    full_text = "".join(text_parts)
    print(f"\n[background_stream_cursor] resumed output: {full_text}")
...
def test_background_stream_retrieve_poll(agent_url):
    ...
    assert resp.status == "completed"
    assert resp.output_text
    print(f"\n[background_stream_retrieve_poll] output: {resp.output_text}")
...
def test_responses_stream_background_retrieve_with_cursor(agent_url):
    ...
    assert resp.status == "completed"
    assert resp.output_text
    print(f"\n[responses_stream_background_cursor] output: {resp.output_text}")
