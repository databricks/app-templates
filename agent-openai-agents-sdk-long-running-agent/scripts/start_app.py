...
class ProcessManager:
    ...
    def run(self, backend_args=None, profile=None):
...
def main():
    ...
    parser.add_argument(
        "--profile",
        help="Databricks config profile (e.g. e2-demo-field) for DB auth",
        default=None,
    )
    ...
    sys.exit(ProcessManager(port=port).run(backend_args, profile=profile))
