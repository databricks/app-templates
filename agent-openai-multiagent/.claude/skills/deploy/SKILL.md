...
databricks bundle run agent_openai_multiagent
```
...
The resource key `agent_openai_multiagent` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind agent_openai_multiagent <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_openai_multiagent
```
...
| App running old code after deploy | Run `databricks bundle run agent_openai_multiagent` after deploy |
