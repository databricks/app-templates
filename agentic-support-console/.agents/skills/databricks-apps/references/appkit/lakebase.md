...
export const appRouter = t.router({
  listItems: publicProcedure.query(async () => {
    const { rows } = await pool.query('SELECT * FROM app_data.items ORDER BY created_at DESC LIMIT 100');
    return rows;
  }),

  createItem: publicProcedure.input(z.object({ name: z.string().min(1) })).mutation(async ({ input }) => {
    const { rows } = await pool.query('INSERT INTO app_data.items (name) VALUES ($1) RETURNING *', [input.name]);
    return rows[0];
  }),
