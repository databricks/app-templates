...
import { getUserFromHeaders } from '../db/queries';
import { isTestEnvironment } from '../../lib/constants';
import { getHostUrl, getHostDomain } from '../utils/databricks-host-utils';
...
export async function getDatabricksUserIdentity(): Promise<string> {
  ...
  const { spawnWithOutput } = await import('../utils/subprocess');
  ...
  if (cliHostCache && Date.now() < cliHostCacheTime + CLI_HOST_CACHE_DURATION) {
    ...
    const { getHostDomain } = await import('../utils/databricks-host-utils');
