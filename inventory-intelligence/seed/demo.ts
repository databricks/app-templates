/**
 * Demo data generator for Inventory Intelligence.
 *
 * Creates deterministic, presentation-ready data with clear per-store narratives:
 *   Chicago Flagship  — crisis store (5 alerts, 3 low)
 *   Seattle Pike      — under pressure (3 alerts, 2 low)
 *   Phoenix Desert    — moderate issues (4 alerts, 2 low)
 *   Austin South      — recovering (1 alert, orders in flight)
 *   Boston Harbor     — healthy reference store (all OK)
 *
 * Run: DATABASE_URL=<pg-url> npm run demo
 *
 * Differences from seed.ts:
 *   - Stock levels are deterministic (controlled scenarios, not random)
 *   - Replenishment queue has specific pending/in-flight orders
 *   - All existing operational data is reset on each run
 *   - Sales history is only seeded when the table is empty (skip re-seed)
 */
...
// ── Reference data (mirrors seed.ts) ─────────────────────────────────────────
...
// Unspecified store×product pairs default to a healthy 21-day supply.
// Stock status is computed as:
//   out_of_stock:  onHand <= 0
//   critical:      onHand <= reorderPoint * 0.5
//   low:           onHand <= reorderPoint
//   ok:            otherwise

const DEMO_STOCK: Record<
  string,
  Record<string, { onHand: number; onOrder: number }>
> = {
  // Chicago Flagship — crisis store: 2 out-of-stock, 3 critical, 3 low
  s1: {
    p01: { onHand: 0, onOrder: 0 }, // Wireless Earbuds Pro     — out_of_stock (reorder=20)
    p13: { onHand: 0, onOrder: 0 }, // Water Bottle             — out_of_stock (reorder=40)
    p08: { onHand: 15, onOrder: 0 }, // Merino Wool Beanie       — critical     (reorder=50, critical<=25)
    p16: { onHand: 8, onOrder: 0 }, // Resistance Band Set      — critical     (reorder=30, critical<=15)
    p21: { onHand: 4, onOrder: 0 }, // Cold Brew Concentrate    — critical     (reorder=60, critical<=30)
    p11: { onHand: 14, onOrder: 0 }, // Cast Iron Skillet        — low          (reorder=20)
    p24: { onHand: 35, onOrder: 0 }, // Mixed Nut Trail Mix      — low          (reorder=50)
    p12: { onHand: 19, onOrder: 0 }, // Bamboo Cutting Board     — low          (reorder=25)
  },

  // Seattle Pike — under pressure: 1 out-of-stock, 2 critical, 2 low
  s3: {
    p03: { onHand: 0, onOrder: 0 }, // Portable Charger         — out_of_stock (reorder=30)
    p08: { onHand: 12, onOrder: 0 }, // Merino Wool Beanie       — critical     (reorder=50, critical<=25)
    p21: { onHand: 5, onOrder: 0 }, // Cold Brew Concentrate    — critical     (reorder=60, critical<=30)
    p25: { onHand: 22, onOrder: 0 }, // Matcha Green Tea         — low          (reorder=35)
    p23: { onHand: 28, onOrder: 0 }, // Raw Honey                — low          (reorder=40)
  },

  // Phoenix Desert — moderate issues: 2 out-of-stock, 2 critical, 2 low
  s5: {
    p07: { onHand: 0, onOrder: 0 }, // Performance Running Shorts — out_of_stock (reorder=35)
    p21: { onHand: 0, onOrder: 0 }, // Cold Brew Concentrate      — out_of_stock (reorder=60)
    p13: { onHand: 10, onOrder: 0 }, // Water Bottle               — critical     (reorder=40, critical<=20)
    p20: { onHand: 2, onOrder: 0 }, // Adjustable Dumbbell Pair   — critical     (reorder=8, critical<=4)
    p15: { onHand: 14, onOrder: 0 }, // Herb Garden Starter Kit    — low          (reorder=20)
    p24: { onHand: 32, onOrder: 0 }, // Mixed Nut Trail Mix        — low          (reorder=50)
  },

  // Austin South — recovering: 1 critical, 2 low (orders already placed)
  s2: {
    p18: { onHand: 12, onOrder: 0 }, // Jump Rope Speed Cable    — critical     (reorder=40, critical<=20)
    p06: { onHand: 25, onOrder: 100 }, // Classic Crewneck Hoodie  — low          (order in flight)
    p22: { onHand: 22, onOrder: 80 }, // Organic Protein Powder   — low          (order in flight)
  },

  // Boston Harbor — healthy reference store: all OK (no overrides needed)
  s4: {},
};

// ── Demo replenishment orders ─────────────────────────────────────────────────
// Pending = awaiting approval in the replenishment queue.
// In-flight = already ordered, showing up under "Orders in Progress".

const DEMO_REPLENISHMENT_ORDERS = [
  // Austin South — orders already placed (match onOrder values above)
  {
    store_id: "s2",
    product_id: "p06",
  ...
  // Boston Harbor — healthy, one order arriving soon
  {
    store_id: "s4",
    product_id: "p19",
    quantity_ordered: 50,
...
function computeStockStatus(onHand: number, reorderPoint: number) {
...
async function upsertStores() {
  for (const s of STORES) {
    await query(
      `INSERT INTO inventory.stores (id, name, region, city, timezone)
...
async function upsertProducts() {
  for (const p of PRODUCTS) {
    await query(
      `INSERT INTO inventory.products (id, sku, name, category, unit_cost_cents, reorder_point, reorder_quantity, lead_time_days)
...
// Only seeds when the table is empty — sales history is slow to regenerate
// and does not need to change between demo resets.
async function ensureSalesHistory() {
  const { rows } = await query(
    `SELECT COUNT(*) AS cnt FROM inventory.sales_transactions`,
  );
  const count = Number((rows[0] as { cnt: string }).cnt);
  if (count > 0) {
    console.log(`Sales history already seeded (${count} rows) — skipping.`);
    return;
  }
  ...
  const rows_: [string, string, number, number, string][] = [];
  ...
  for (let i = 0; i < rows_.length; i += BATCH_SIZE) {
    const batch = rows_.slice(i, i + BATCH_SIZE);
  ...
  console.log(`Sales history seeded (${total} transactions, 90 days).`);
}

async function resetDemoStockLevels() {
  await query(`DELETE FROM inventory.stock_levels`);

  for (const store of STORES) {
    const storeOverrides = DEMO_STOCK[store.id] ?? {};

    for (const product of PRODUCTS) {
      const override = storeOverrides[product.id];
      let onHand: number;
      let onOrder: number;

      if (override) {
        onHand = override.onHand;
        onOrder = override.onOrder;
      } else {
        // Default: healthy ~21 days of supply
        const baseSales = BASE_DAILY_SALES[store.id]?.[product.id] ?? 1.0;
        const avgDaily = Math.max(1, Math.round(baseSales));
        onHand = avgDaily * 21 + Math.floor(Math.random() * (avgDaily * 7));
        onOrder = 0;
      }
  ...
  console.log(
    `Demo stock levels set (${STORES.length * PRODUCTS.length} store×product pairs).`,
  );
}

async function resetDemoReplenishmentOrders() {
  await query(`DELETE FROM inventory.replenishment_orders`);

  for (const o of DEMO_REPLENISHMENT_ORDERS) {
  ...
  console.log(
    `Demo replenishment orders set (${DEMO_REPLENISHMENT_ORDERS.length} orders).`,
  );
}

// ── Gold tables ───────────────────────────────────────────────────────────────
// Mirrors what Sync Tables provides in production. Computed from the OLTP data
// so the app works immediately without running the full DLT pipeline.
...
async function ensureGoldSchema() {
  await query(`CREATE SCHEMA IF NOT EXISTS gold`);
  await query(`
    CREATE TABLE IF NOT EXISTS gold.inventory_overview_sync (
      store_id TEXT NOT NULL, store_name TEXT NOT NULL, region TEXT NOT NULL,
      product_id TEXT NOT NULL, sku TEXT NOT NULL, product_name TEXT NOT NULL,
      category TEXT NOT NULL, unit_cost_cents INTEGER NOT NULL,
      reorder_point INTEGER NOT NULL, reorder_quantity INTEGER NOT NULL,
      lead_time_days INTEGER NOT NULL, quantity_on_hand INTEGER NOT NULL,
      quantity_on_order INTEGER NOT NULL, last_counted_at TIMESTAMPTZ NOT NULL,
      avg_daily_units_30d NUMERIC(10,4) NOT NULL, units_7d INTEGER NOT NULL,
      units_30d INTEGER NOT NULL, days_of_supply NUMERIC(10,1),
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS gold.replenishment_recommendations_sync (
      ...
      forecast_30d_units NUMERIC(10,2) NOT NULL, recommended_order_qty INTEGER NOT NULL,
      confidence TEXT NOT NULL, model_used TEXT NOT NULL, generated_at TIMESTAMPTZ NOT NULL,
      PRIMARY KEY (store_id, product_id)
    )
  `);
  await query(`
    CREATE TABLE IF NOT EXISTS gold.low_stock_alerts_sync (
      store_id TEXT NOT NULL, store_name TEXT NOT NULL, region TEXT NOT NULL,
      product_id TEXT NOT NULL, sku TEXT NOT NULL, product_name TEXT NOT NULL,
      category TEXT NOT NULL, quantity_on_hand INTEGER NOT NULL,
      quantity_on_order INTEGER NOT NULL, reorder_point INTEGER NOT NULL,
      reorder_quantity INTEGER NOT NULL, lead_time_days INTEGER NOT NULL,
      avg_daily_units_30d NUMERIC(10,4) NOT NULL, days_of_supply NUMERIC(10,1),
      stock_status TEXT NOT NULL,
      PRIMARY KEY (store_id, product_id)
    )
  `);
...
async function refreshGoldTables() {
  const storesResult = await query(
    `SELECT id, name, region FROM inventory.stores`,
  );
  ...
  for (let i = 0; i < overviewRows.length; i += BATCH) {
    ...
    const placeholders = batch
      .map(
        (_, j) =>
          `(${Array.from({ length: 19 }, (__, k) => `$${j * 19 + k + 1}`).join(",")})`,
      )
      .join(",");
    await query(
      `INSERT INTO gold.inventory_overview_sync VALUES ${placeholders}
  ...
  for (let i = 0; i < recRows.length; i += BATCH) {
    ...
    const placeholders = batch
      .map(
        (_, j) =>
          `(${Array.from({ length: 7 }, (__, k) => `$${j * 7 + k + 1}`).join(",")})`,
      )
      .join(",");
    await query(
      `INSERT INTO gold.replenishment_recommendations_sync VALUES ${placeholders}
       ON CONFLICT (store_id, product_id) DO UPDATE SET
         forecast_30d_units=EXCLUDED.forecast_30d_units,
  ...
  for (let i = 0; i < alertRows.length; i += BATCH) {
    ...
    const placeholders = batch
      .map(
        (_, j) =>
          `(${Array.from({ length: 15 }, (__, k) => `$${j * 15 + k + 1}`).join(",")})`,
      )
      .join(",");
    await query(
      `INSERT INTO gold.low_stock_alerts_sync VALUES ${placeholders}
       ON CONFLICT (store_id, product_id) DO UPDATE SET
         store_name=EXCLUDED.store_name, region=EXCLUDED.region, sku=EXCLUDED.sku,
         product_name=EXCLUDED.product_name, category=EXCLUDED.category,
         quantity_on_hand=EXCLUDED.quantity_on_hand, quantity_on_order=EXCLUDED.quantity_on_order,
  ...
  console.log(
    `Gold tables refreshed: ${overviewRows.length} overview rows, ${recRows.length} recommendations, ${alertRows.length} alerts.`,
  );
}
...
async function main() {
  ...
  await resetDemoStockLevels();
  await resetDemoReplenishmentOrders();
  await ensureGoldSchema();
  await refreshGoldTables();
...
Store summary:
  Chicago Flagship  — 2 out-of-stock, 3 critical, 3 low  (crisis store)
  Seattle Pike      — 1 out-of-stock, 2 critical, 2 low  (under pressure)
  Phoenix Desert    — 2 out-of-stock, 2 critical, 2 low  (moderate issues)
  Austin South      — 1 critical, 2 low (orders in flight)
  Boston Harbor     — all healthy (reference store)
`);
}
