...
echo "=========================================="
echo "Test User Authorization - Databricks App"
echo "=========================================="
echo ""
...
# Run the OAuth token generation and capture only stdout (the token)
# Let stderr go to console so user sees progress messages
token=$(python3 "$SCRIPT_DIR/generate_oauth_token.py" \
    --host "$host" \
    --scopes "$scopes" \
...
# Step 6: Run the test
echo "Step 4: Testing user authorization with MCP server..."
echo ""

cd "$PROJECT_ROOT"

python3 "$SCRIPT_DIR/test_user_auth.py" \
    --host "$host" \
    --token "$token" \
    --app-url "$app_url"
...
if [ $? -eq 0 ]; then
    echo ""
    echo "=========================================="
    echo "✓ User Authorization Test Complete!"
    echo "=========================================="
else
    echo ""
    echo "=========================================="
    echo "✗ User Authorization Test Failed"
    echo "=========================================="
    exit 1
fi
